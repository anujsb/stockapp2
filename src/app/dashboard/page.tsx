      ...
      if (portfolioResponse.ok) {
        ...
        const sorted = portfolioData.portfolio
          .map((item: PortfolioItem) => ({
            ...item,
            pnl: (item.stock.current_price - item.avg_purchase_price) * item.quantity,
            pnlPercent: ((item.stock.current_price - item.avg_purchase_price) / item.avg_purchase_price) * 100
          }))
